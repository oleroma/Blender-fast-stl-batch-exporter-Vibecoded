...
### 2. Adaptive Dual-Path Execution (New)
The exporter now intelligently analyzes your preset configuration to select the optimal execution route:
* **Synchronous Bypass:** If you are running a standard export without any node or modifier overrides, the exporter evaluates the Depsgraph natively and writes the STLs instantly in the main thread, accompanied by Blender's native cursor progress bar.
* **Headless Background Worker:** If destructive permutations are detected, the exporter seamlessly falls back to its isolated subprocess architecture to protect your active project file from memory leaks or infinite loops.
...
### 9. Headless Export & Crash-Proof UI
...
* **Instant Boot:** Utilizes `--factory-startup` to skip loading addons, themes, and UI elements, drastically accelerating the background launch time.
* **Granular Progress Bar:** The UI correctly computes total `Objects × Permutations` to provide a highly granular progress bar, preventing UI stalls on large batches.
* **Zero-Delay UI Teardown:** Main thread gracefully force-terminates the subprocess the exact millisecond the export concludes, bypassing Blender's slow native memory teardown and un-locking your UI instantly.
* **Instant Kill Switch:** Clicking "Cancel Export" (or pressing the `ESC` key) immediately terminates the background process and deletes the temporary file, ensuring no frozen UI states or corrupted project files.[cite: 3]

### 10. Refined UX & Undo History (New)
...
7. Hit **Export**.[cite: 3] The headless script evaluates the permutations in the background, updates your progress bar, and organizes the STL files into their appropriate paths.[cite: 3]
